  ...
  const game = new Phaser.Game({
    ...
    callbacks: {
      postBoot: (bootedGame) => {
        ...
        // scene before them in the flow (MainMenu -> ModeSelect ->
        // Lobby/InternetLobby -> GameTable, MainMenu -> RulesScene,
        // MainMenu -> SettingsScene, MainMenu -> ProfileScene, MainMenu
        // -> ChuoScene, see each scene's own docstring) -- registered
        // here (autoStart=false) purely so Phaser's scene manager knows
        // the keys exist to start later; each gets its real init data
