...
import { computeMultiplayerMenuLayout } from '../MultiplayerMenuLayout.js';
...
describe('computeMultiplayerMenuLayout', () => {
  it('returns exactly local, internet, then back -- no LAN placeholder, ever', () => {
    for (const viewport of [NARROW_PHONE_PORTRAIT, MID_TABLET_PANEL, WIDE_DESKTOP]) {
      const layout = computeMultiplayerMenuLayout(viewport, ZERO_INSETS);
      expect(layout.buttons.map((b) => b.id)).toEqual(['local', 'internet', 'back']);
  ...
  it('places the title above the button stack, and stacks buttons with no overlap', () => {
    const layout = computeMultiplayerMenuLayout(MID_TABLET_PANEL, ZERO_INSETS);
  ...
  it('puts "back" clearly separated below the two mode buttons', () => {
    const layout = computeMultiplayerMenuLayout(MID_TABLET_PANEL, ZERO_INSETS);
    const [local, internet, back] = layout.buttons;
    const gapBetweenModes = internet.y - (local.y + local.height);
  ...
  it('enforces the minimum touch target on every button at a tiny viewport', () => {
    const layout = computeMultiplayerMenuLayout({ width: 320, height: 400 }, ZERO_INSETS);
  ...
  it('grows scale/size when the viewport grows', () => {
    const phoneLayout = computeMultiplayerMenuLayout(NARROW_PHONE_PORTRAIT, ZERO_INSETS);
    const tabletLayout = computeMultiplayerMenuLayout(MID_TABLET_PANEL, ZERO_INSETS);
  ...
  it('shifts every element inward when safe-area insets are applied', () => {
    const noInsets = computeMultiplayerMenuLayout(MID_TABLET_PANEL, ZERO_INSETS);
    const withInsets = computeMultiplayerMenuLayout(MID_TABLET_PANEL, {
      top: 40,
      right: 20,
      bottom: 20,
      left: 20,
