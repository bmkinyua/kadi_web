...
| Main Menu | `scenes.py`'s `MainMenuScene` | **Done, narrowed scope — updated this session.** Web `MainMenuScene` offers "Play vs AI", "Multiplayer", "Profile", "Settings", "How to Play", and "Chuo" (all six enabled, all routing to real scenes). "Play vs AI" now routes DIRECTLY to `GameConfigScene(vsAi=true)` (previously to the old `ModeSelectScene`, itself only a "which mode" picker — see below); "Multiplayer" is new this session, routing to `MultiplayerMenuScene`. "Continue Game" and "Quit" remain dropped, not disabled: Continue depends on a local-save-file concept with no web equivalent or planned row, and there's no web equivalent of Quit. |
| Multiplayer Menu | `scenes.py`'s `MultiplayerMenuScene` (class at line 4303) | **Done, new this session.** New `MultiplayerMenuScene.ts` + `layout/MultiplayerMenuLayout.ts` (replaces the old `ModeSelectLayout.ts`/`ModeSelectScene.ts`, which previously did this screen's job under the wrong name — see git history). Reached from Main Menu's "Multiplayer" button. Exactly two buttons plus Back: "Local Multiplayer" (routes to `GameConfigScene(vsAi=false)`) and "Internet Multiplayer" (routes to `InternetLobbyScene`, untouched). LAN is **permanently out** (no placeholder button, ever — asserted in `MultiplayerMenuLayout.test.ts`). Back routes to `MainMenuScene`, matching `MultiplayerMenuScene`'s own Back on the PC side exactly. |
| Game Config (per-game setup screen) | `scenes.py`'s `ModeSelectScene` (class at line 1542, parameterized by `vs_ai: bool`) | **Done for Play vs AI end-to-end; Local Multiplayer's actual gameplay is a known, reported gap — see "Known Issues" below.** New `GameConfigScene.ts` + `layout/GameConfigLayout.ts`, the real per-game configuration screen the old `ModeSelectScene.ts` had deliberately skipped (this row used to read "**Done, narrowed scope**" for exactly that reason — that status was closed out this session, not left stale). One shared scene for both entry points, matching `ModeSelectScene(vs_ai: bool)` exactly: reached directly from Main Menu's "Play vs AI" (`vsAi=true`) or from `MultiplayerMenuScene`'s "Local Multiplayer" (`vsAi=false`). Covers: "Your Name" (new free-text field widget — no other web scene needed one before this), opponent count **1–5** (both modes; see this row's own correction note below), AI Difficulty + MSOMI attach (vs-AI only, MSOMI reusing `ChuoScene`'s `MsomiStore.listModels()`/`loadModel()` and `msomi/trainer.ts`'s `validateModel()` rather than reimplementing), Elimination Mode toggle (both modes) + its "AI-only continue"/"End the game" sub-toggle (vs-AI + Elimination Mode only), and — Local Multiplayer only — up to 5 hot-seat opponent name fields that grow/shrink live with the opponent-count selection. Back always goes straight to `MainMenuScene` in BOTH modes, matching `ModeSelectScene`'s own Back on the PC side exactly (never back to `MultiplayerMenuScene`, even for Local Multiplayer, which was reached through it). Reuses `SettingsScene.ts`'s established scroll/render/click-to-focus discipline (`layout/scrollPhysics.ts`, destroy-and-rebuild-per-`render()`). **Play vs AI — Start Game is fully wired**: builds a real `CreateGameSettings` (opponent count → `ai_count`, difficulty, elimination mode + `elimination_ai_only_continue`, embedded MSOMI model) and the typed name, hands both to `LobbyScene` (extended this session to accept `gameSettings`/`playerName` instead of always using its old fixed Quick Play settings) — verified against the real server via the existing `gameIntegration.test.ts`-style path. **Correction versus an earlier draft of this task's own handoff prompt:** opponent count is **1–5, never 0** — the prompt described "0 opponents" as how AI Spectator Mode is reached; checked directly against `scenes.py`'s `_opp_buttons` loop (`for i, n in enumerate([1,2,3,4,5])`) and `GameManager.is_ai_spectator_mode` (`core/game_manager.py`), which is a MID-GAME property (true once every human has finished in Elimination Mode) rather than a setup-time choice — built against the real source, not the prompt's inaccurate paraphrase of it. Unit-tested: `GameConfigLayout.test.ts` (10 tests: opponent-count range, mode-conditional rows, hot-seat field count tracking, scroll behavior, button ordering). |
| Local Multiplayer — actual pass-and-play gameplay | `scenes.py`'s `ModeSelectScene(vs_ai=False)` → `PassAndPlayOverlay` hot-seat turn handling | **Blocked on a genuine server-side architecture gap — reported, not silently patched around, per this session's own handoff instructions.** Investigated directly (not assumed): `server/game_room.py`'s entire roster model is **one human seat per server connection** (`self.member_names`, keyed by `conn_id`, built in `start_game()`) — there is no existing path for a single browser tab/connection to own several human seats, confirmed independently by `packages/renderer/src/profileData.ts`'s own pre-existing comment ("'lan'/'hot_seat' are never produced by this web client's server ... real LAN multiplayer and hot-seat are both out of scope for the web build per §9"). `GameConfigScene(vsAi=false)` collects every PC-parity input (hot-seat names, opponent count, elimination mode) and is otherwise fully usable/testable, but its Start Game button surfaces an explicit in-scene message ("Local Multiplayer needs a server-side update...") rather than attempting a `create_game` that the server can't actually honor. Closing this gap for real needs a genuine design decision — how a single connection's several human seats get reconnect tokens, how `PassAndPlayOverlay`'s privacy/handoff screen maps onto `GameTableScene`'s existing single-viewpoint rendering — not a mechanical fix, so it's deliberately left open here rather than guessed at unilaterally. |
...
| Chuo / MSOMI (train-your-own-AI) | `scenes.py`'s `ChuoScene`, `msomi_trainer` | **Done.** New `ChuoScene.ts` + `layout/ChuoLayout.ts` (pure, unit-tested — 16 tests in `ChuoLayout.test.ts`), reached from Main Menu's now-**enabled** "Chuo" button. All four real tabs ported (Data, Features, Model, Train & Results) — one unified scrollable row list per active tab, rather than the PC's pinned-buttons-above-a-list layout (a phone-width column doesn't need pinned headers the way a desktop window does; same simplification `SettingsLayout.ts` already made). **Storage decision (was "still open" here as of the previous revision of this row — that was stale; decided this session, not left unresolved):** two backends behind one shared `MsomiStore` interface (`packages/adapter-interface/src/MsomiStore.ts`) — the File System Access API where the browser supports it (`FileSystemAccessMsomiStore.ts`, a real native file/folder picker, genuinely capable of reading the PC desktop client's own `logs/`/`msomi_models/` folder directly if pointed at it) falling back to IndexedDB elsewhere (`IndexedDbMsomiStore.ts`), chosen via feature detection in `createMsomiStore.ts` (checks for `showDirectoryPicker`, never user-agent sniffing). Both backends are genuinely unit-tested — 31 tests total (`IndexedDbMsomiStore.test.ts` against real `fake-indexeddb`; `FileSystemAccessMsomiStore.test.ts` against small in-memory fakes implementing the same narrow handle interface the real code calls, since no live browser is available in this environment — manual browser-only test steps for the real file-picker path are documented at the bottom of `FileSystemAccessMsomiStore.ts` itself). **Trainer port:** `msomi_trainer.train()`'s plain-NumPy conditional-logit gradient descent is reimplemented directly in TypeScript (`packages/renderer/src/msomi/trainer.ts`) with ordinary arrays — no new dependency (`mathjs` was considered and rejected as overkill for ≤13 features); parity with the real Python trainer is verified directly, not assumed — `trainer.test.ts` trains on a fixed decision set and asserts the TS output matches `core/msomi_trainer.py`'s own `train()` output on the same input to 6 decimal places (the fixture was generated by actually running the Python trainer once, not hand-derived). 21 tests total for the trainer/loader/scorer/validator. **One genuine web-side addition beyond the PC scope:** a "Saved models" list with per-model Load/Delete on the Train & Results tab — the PC `ChuoScene` has no delete affordance anywhere (confirmed by grepping the whole codebase, not assumed), added here as the natural low-cost counterpart of local save/load, not because it was asked for as a literal port. **Deliberately not ported this pass** (see `ChuoLayout.ts`'s own header): the MSOMI-attach illustration splice (`help_msomi_attach.png`) on the Help overlay's "Using a trained model in a game" section — the overlay itself (title/sections text) was closed out in a later session, see the "Help Overlay" row below. Also simplified vs. the PC's `NumberBox`: the two numeric fields (human weight, training iterations) use +/- steppers only, no click-to-focus keyboard digit entry — both fields are coarse-grained enough (step 1 and step 50 respectively) that this doesn't cost real precision, and it avoids this already-large scene needing its own copy of `SettingsScene.ts`'s keyboard-focus state machine for two fields. **Explicitly still out of scope, not this delivery's job:** attaching a trained model to an AI opponent (`MSOMIPickerWidget`/`ModeSelectScene`'s own "Attach Model..." flow on the PC side) — Chuo's own "Load" button validates a saved model and shows its weights, which is useful standalone, but doesn't wire into gameplay; that's a separate task scoped to `ModeSelectScene.ts`, which this delivery does not touch. |
| Help Overlay (contextual "?" button + popup, all five screens) | `rendering/widgets.py`'s `HelpOverlay` class (~line 1404) + `make_help_button()` (~line 1647); five real usages in `scenes.py` — Main Menu (~875), Mode Select/`GameConfigScene` (~1644), Chuo (~3764, closing that screen's own previously-deferred gap — see the Chuo row above), Multiplayer Menu (~4370), Gameplay/`GameTableScene` (~6743) | **Done — all five screens, text content ported verbatim.** New shared `HelpOverlay.ts` (the Phaser widget: button, glow, modal scrollable panel, wheel + touch-drag scroll reusing `layout/scrollPhysics.ts`, Escape/outside-click/close-button dismissal), `layout/HelpOverlayLayout.ts` (pure panel/button/section-flow math, unit-tested — 16 tests), `idleGlow.ts` (the 30s-trigger/3s-duration "stuck?" nudge as a pure state machine + thin stateful wrapper, same shape as `statusText.ts`'s `StatusTextGuard` — 14 unit tests covering trigger timing, repeat cadence, reset-on-activity, and full suppression while open), and `layout/HELP_CONTENT.ts` (the real per-screen copy, extracted directly from `scenes.py`'s five `HELP_SECTIONS`/`_help_sections()` — nothing paraphrased). Two screen-specific behaviors preserved, not simplified away: `GameConfigScene` fully suppresses the button/overlay while its own MSOMI-picker modal is open (`setEnabled()`), and `GameTableScene`'s idle-glow only accrues while it's actually the human's turn and the game isn't paused (`update(delta, shouldAccrueIdle)`), matching `GameplayScene.update()`'s own one deviation from every other help-enabled screen. **"?" button placement is a fresh choice, not a per-screen port:** PC hand-places each button next to that screen's own `AudioControls` sfx/music icons; this web client has no such icon row anywhere (confirmed by grep before writing), so all five screens instead anchor to one consistent spot — the top-right corner of the screen's own safe content rect. **Not ported this pass, flagged rather than dropped silently:** the two image sections (Chuo's MSOMI-attach illustration, Gameplay's drag-illustration) — this web client has no image-asset loading pipeline anywhere (no `load.image`/`preload`, no `public`/`static` folder, nothing in `vite.config.ts`; `GameTableScene.ts`'s own header already notes card art is drawn as code specifically because no such pipeline exists), so building one just for two help illustrations was left for a separately-scoped task. The exact insertion point for each is marked with a comment in `HELP_CONTENT.ts`. Verified: real Playwright screenshots (Chromium, driving the actual production build against a live local server — not a mockup) across three of the five screens (Main Menu, `GameConfigScene`, Chuo), including a genuine unattended 30-second wait that captured the idle-glow nudge actually firing in a live browser. One real bug this caught (not in a unit test): a long overlay title (e.g. "Play vs AI — Quick Guide") could visually collide with the close button on a narrow phone-width panel — PC never hits this since its own panel is never narrower than ~944px, but this renderer's panel can shrink to a 260px floor for phone viewports PC never had to support. Fixed with a word-wrap constraint on the title (see `HelpOverlay.ts`'s own inline comment); confirmed fixed in a follow-up screenshot showing the title correctly wrapping to two lines instead. **One content/UI mismatch, resolved with the user rather than silently edited or silently shipped:** Multiplayer Menu's ported HELP_SECTIONS text (PC's own real copy) describes a "LAN Multiplayer" option, but this web client's own `MultiplayerMenuScene.ts` has no LAN button at all ("No LAN button, ever" — that file's own header). Flagged rather than fixed unilaterally; resolved as a follow-up decision by adding one clearly-marked extra line immediately after PC's LAN paragraph (`HELP_CONTENT.ts`'s `MULTIPLAYER_MENU_HELP`, with its own inline comment distinguishing it from the verbatim port) noting LAN isn't available on web and pointing to the desktop app — a cross-sell, not a correction of PC's own text. Confirmed rendering correctly with a follow-up screenshot. Full suite re-run clean after this delivery: 353/353 renderer vitest (up from 337), 2/2 client-core (real-server integration), 43/43 adapters/web, 47/47 pytest, clean typecheck, real production build succeeds. |
...
_(none open. Closed this session: the new Help Overlay's title text
could visually collide with its own close button on a narrow
phone-width panel — found via this session's own real Playwright
screenshot verification (a live Chromium render, not a mockup),
same "caught by a live screenshot, not by any test" pattern as the
Chuo tab-bar bug below. Root cause: `HelpOverlayLayout.ts`'s title
line, like PC's own `draw()` (`widgets.py` ~1591), positions the
title at a fixed x with no width clamp against the close button —
harmless on PC since its panel is never narrower than ~944px
(`min(680, 1024-80)`, PC's lowest supported resolution), but this
renderer's panel can shrink to a 260px floor for phone viewports PC
never had to handle, where a longer title (e.g. "Play vs AI — Quick
Guide") reaches the close button. Fixed with a word-wrap constraint
on the title in `HelpOverlay.ts`; confirmed fixed with a follow-up
screenshot of the same screen showing the title correctly wrapping
to two lines instead of overlapping.)_

_(Also closed previously: `ChuoScene.ts`'s tab bar and Back
